...
const products=[
 ...
 {id:3,
  ...
  category:"Skincare",
  price:77000,
  images:["Imagenes/Productos/Skincare/Dúo de Hyaluronic Acid 2% + B5 & AHA 30% + BHA 2% Peeling Solution de The Ordinary®.jpg",
          "Imagenes/Productos/Skincare/Dúo de Hyaluronic Acid 2% + B5 & AHA 30% + BHA 2% Peeling Solution de The Ordinary®.jpg"]},
...
function renderFilters()
{
 ...
 box.innerHTML=`<button class="category-filter ${activeCategory==="Todas"?"active":""}" data-cat="Todas">TODAS</button>`+categories.map(c=>`<button class="category-filter ${activeCategory===c?"active":""}" data-cat="${c}">${c.toUpperCase()}</button>`).join("");
 box.querySelectorAll(".category-filter").forEach(b=>b.onclick=()=>{activeCategory=b.dataset.cat;renderFilters();renderProducts()});
}
